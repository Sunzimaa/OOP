...
def read_from_file_into_list(filename: str) -> list:
    ...
    """
    list_of_clients = []
    for client in open(filename):
        name, bank, account_age, starting_amount, current_amount = client.split(',')
...
def filter_by_bank(filename: str, bank: str) -> list:
    ...
    for client in read_from_file_into_list(filename):
        if client.bank == bank:
            clients_in_bank.append(client)
    return clients_in_bank
    pass
...
def largest_earnings_per_day(filename: str) -> Optional[Client]:
    ...
    """
    largest_earnings = sorted(read_from_file_into_list(filename), key=lambda client: (-client.earnings_per_day(), -client.account_age), reverse=True)[0]
    if largest_earnings.earnings_per_day() >= 0:
        return None
    else:
        return largest_earnings
    pass
...
def largest_loss_per_day(filename: str) -> Optional[Client]:
    ...
    """
    largest_loss = sorted(read_from_file_into_list(filename), key=lambda client: (client.earnings_per_day(), -client.account_age), reverse=True)[0]
